...
-- Track if we're inside an allowed area
local allowedArea = false

local function UpdateAllowedArea()
    local z  = GetRealZoneText() or GetZoneText() or ""
    local sz = GetSubZoneText() or ""
    -- exact matches first
    allowedArea = ALLOWED[z] or ALLOWED[sz] or false
    -- tolerant fallbacks for subzones/variants
    if not allowedArea then
        if strfind(z, "Karazhan", 1, true) or strfind(sz, "Karazhan", 1, true) then
            allowedArea = true
        elseif strfind(z, "Rock of Desolation", 1, true) or strfind(sz, "Rock of Desolation", 1, true) then
            allowedArea = true
        end
    end
    -- leaving the area should clear UI/sound state
    if not allowedArea then
        if dontMoveWarningFrame then dontMoveWarningFrame:Hide() end
        if dontMoveGlow then dontMoveGlow:Hide() end
        dontMovePlaying = false
    end
end

-- Keep the flag current
...
zoneWatcher:RegisterEvent("MINIMAP_ZONE_CHANGED")
zoneWatcher:SetScript("OnEvent", UpdateAllowedArea)
UpdateAllowedArea()

-- Your existing function, gated by zone
local function CheckDontMove()
    if not allowedArea then
        -- ensure hidden if called out of bounds
        dontMoveWarningFrame:Hide()
        dontMoveGlow:Hide()
        dontMovePlaying = false
        return
